...
- 跟读/接句已支持服务端文本评分 fallback，并可上传浏览器录音到 Supabase Storage。

## 下一阶段开发计划

优先补齐 V1 验收闭环，再做部署前检查：

1. 真实 ASR 接入：把录音文件转写为文本，继续复用现有文本评分 fallback。
2. 生词复习强化：完善复习结果更新、到期队列和间隔重复参数。
3. 管理导入收尾：补媒体文件路径/Storage 上传说明，确认字幕编辑和导入任务状态可追踪。
4. 验收与部署：跑 `typecheck`、`lint`、`build`，做桌面和移动端学习页冒烟检查。
